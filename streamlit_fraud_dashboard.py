...
def main():
    ...
    with col1:
        st.metric("Altman Z-Score", f"{z_score:.3f}", f"{z_risk} Risk")
        st.markdown(f"<p style='color: {z_color}; font-weight: 600; text-align: center;'>Risk Level: {z_risk}</p>", unsafe_allow_html=True)
    
    with col2:
        st.metric("Beneish M-Score", f"{m_score:.3f}", f"{m_risk} Risk")
        st.markdown(f"<p style='color: {m_color}; font-weight: 600; text-align: center;'>Risk Level: {m_risk}</p>", unsafe_allow_html=True)
    
    with col3:
        st.markdown(f"""
        <div style='background: rgba(30,41,59,0.9); padding: 1.5rem; border-radius: 12px; text-align: center;'>
            <h4 style='color: #f1f5f9;'>Analysis Year</h4>
            <h2 style='color: #3b82f6;'>{selected_year}</h2>
        </div>
        """, unsafe_allow_html=True)
